                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (image != null) {
                            ...
                            final Bitmap scanBmp = TesseractUtil.getInstance(CameraView.this.getContext()).catchPhoneRect(TesseractUtil.rotateToDegrees(bmp, 90), hintImage);
                            if (scanBmp == null) {
                                isScanning = false;
                                return;
                            }
                            ...
                            //开始识别
                            TesseractUtil.getInstance(CameraView.this.getContext()).scanNumber(scanBmp, new SimpleCallback() {
                                @Override
                                public void response(String result) {
                                    ...
                                    final String tresult = getTelnum(result.replace(" ", ""));
                                    if (!TextUtils.isEmpty(tresult)) {
                                        //检索结果中是否包含手机号
                                    textResult.post(new Runnable() {
                                        @Override
                                        public void run() {
                                            textResult.setText("扫描结果:"+tresult);
                                        }
                                    });
                                        Toast.makeText(CameraView.this.getContext(),"识别的手机号："+ result,Toast.LENGTH_SHORT);
